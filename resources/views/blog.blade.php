...
@section("main")
    ...
    @foreach($posts as $post)
        <article class="card mb-4 flex-row flex-wrap">
            <img src="/image/placeholder.png" alt="{{ $post->title }}" title="{{ $post->title }}" class="mw-25 w-25 mx-3" />
            <div class="card-body px-2 w-25 ml-4 d-flex flex-column">
                ...
                <p class="card-text mw-25 pr-3">{{ $post->content }}</p>
                <a href="#" class="btn btn-outline-primary ml-auto">Lees meer</a>
            </div>
        </article>
    @endforeach
    {{ $posts->render() }}
